    ...
    func getList(count: Int, options: [String: Any] = [:]) -> Observable<ListDto<T>> {
        return request.getList(count: count, options: options)
            ...
            .map({(response: HttpResponse) -> ListDto<T> in
                ...
                self.updateGlobalData(response)//                return ListDto(data: listResponse.data, pagination: response.pagination)
                return ListDto(data: listResponse.data, total: listResponse.total)
            })
    }
